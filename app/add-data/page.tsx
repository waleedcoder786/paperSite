...
const WhiteAdminPanel = () => {
  ...
  const toggleNewClass = () => {
    const nextState = !isNewClass;
    setIsNewClass(nextState);
    if (nextState) {
      setIsNewSubject(true);
      setIsNewChapter(true);
      setIsNewTopic(true);
      setFormData(prev => ({...prev, classId: "", subjectName: "", chapterName: "", topic: ""}));
    } else {
  ...
  const toggleNewSubject = () => {
    const nextState = !isNewSubject;
    setIsNewSubject(nextState);
    if (nextState) {
      setIsNewChapter(true);
      setIsNewTopic(true);
      setFormData(prev => ({...prev, subjectName: "", chapterName: "", topic: ""}));
    }
  };

  const toggleNewChapter = () => {
    const nextState = !isNewChapter;
    setIsNewChapter(nextState);
    if (nextState) {
      setIsNewTopic(true);
      setFormData(prev => ({...prev, chapterName: "", topic: ""}));
    }
  };
            ...
            <div className="bg-white rounded-md p-8 shadow-xl border border-slate-100 grid grid-cols-1 md:grid-cols-5 gap-6">
                <div className="space-y-2">
                  <div className="flex justify-between px-1">
                    ...
                    <button onClick={toggleNewClass} className="text-[9px] font-black text-blue-600 uppercase">{isNewClass ? 'Select' : 'New+'}</button>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between px-1">
                    ...
                    <button onClick={toggleNewSubject} className="text-[9px] font-black text-blue-600 uppercase">{isNewSubject ? 'Select' : 'New+'}</button>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between px-1">
                    ...
                    <button onClick={toggleNewChapter} className="text-[9px] font-black text-blue-600 uppercase">{isNewChapter ? 'Select' : 'New+'}</button>
                  </div>
            ...
            <div className="w-full mx-auto space-y-6">
                ...
                {inputMode === "manual" ? (
                  <form onSubmit={handleSaveManual} className="bg-white rounded-md p-10 shadow-2xl border border-slate-300 space-y-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="col-span-full pt-4 ">
                          ...
                          <div className="flex gap-4">
                            {['A','B','C','D'].map(a => (
                              <button key={a} type="button" onClick={() => setFormData({...formData, answer: a})} className={`flex-1 py-4 rounded-md font-black transition-all border ${formData.answer === a ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-slate-50  border border-slate-300 text-slate-400 hover:bg-slate-100'}`}>{a}</button>
                            ))}
                          </div>
                        </div>
                      </div>
                    ...
                    {type !== 'mcq' && (
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Solution</label>
                    ...
                    <button disabled={loading} className="w-full bg-slate-900 text-white mt-4 py-5 rounded-md font-black uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-slate-800 transition-all shadow-xl active:scale-95 disabled:opacity-50">
                  ...
                  <div className="bg-white rounded-md p-20 border-2 border-dashed border-slate-300 flex flex-col items-center justify-center space-y-8">
                    ...
                    <div className="text-center"><p className="text-lg font-black text-slate-800">File Import for "{formData.topic || 'Select Topic'}"</p></div>
